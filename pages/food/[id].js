...
export default function article( {frontmatter, id, content, paths}) {
    ...
    return (
      <div className="md:grid md:grid-cols-4 w-full max-w-7xl pt-10">
        <article className="prose prose-stone col-span-3 pl-6 min-w-full" dangerouslySetInnerHTML={{ __html: marked(content)}}>
...
export async function getStaticPaths() {
    const files = fs.readdirSync(path.join('content/food'))
...
export async function getStaticProps({params: {id}}) {
    ...
    const markdownWithMeta = fs.readFileSync(path.join('content/food', id + '.md'), 'utf-8')
